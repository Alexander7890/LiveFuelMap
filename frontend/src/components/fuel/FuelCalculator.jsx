...
function PresetButton({ active, children, onClick }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`rounded-full px-3 py-1.5 text-xs font-black transition hover:-translate-y-0.5 ${
        active
...
export default function FuelCalculator() {
  ...
  const priceSource = useMemo(() => {
    if (manualPrice) return { price: Number(manualPrice), label: "ручна ціна" };
    ...
    return Number.isFinite(avg) ? { price: avg, label: "середня ціна по мережі" } : { price: 0, label: "ціни немає" };
  }, [manualPrice, station, fuelCode, stations]);
  ...
  return (
    <Card>
      <div className="flex items-center gap-3">
        ...
        <div>
          <h3 className="text-lg font-black">Калькулятор витрат</h3>
          <p className="text-sm text-slate-500">Автоматично підтягує останню ціну для АЗС і пального.</p>
        </div>
      </div>

      <div className="mt-5 grid gap-4 md:grid-cols-2">
        <Select label="АЗС" value={station?.id || ""} onChange={event => setStationId(event.target.value)}>
          {stations.map(item => <option key={item.id} value={item.id}>{item.name} · {item.address}</option>)}
        </Select>
        <Select label="Пальне" value={fuelCode} onChange={event => setFuelCode(event.target.value)}>
        ...
        <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-3 dark:border-slate-800 dark:bg-slate-900/50">
          <Input type="number" min="1" label="Відстань, км" value={distance} onChange={event => setDistance(event.target.value)} />
        ...
        <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-3 dark:border-slate-800 dark:bg-slate-900/50">
          <Input type="number" min="0.1" step="0.1" label="Витрата, л/100 км" value={consumption} onChange={event => setConsumption(event.target.value)} />
          <div className="mt-3 flex flex-wrap gap-2">
        ...
        <Input type="number" min="0" step="0.01" label="Своя ціна, грн/л" value={manualPrice} onChange={event => setManualPrice(event.target.value)} hint="Необов'язково. Якщо пусто, береться ціна з БД." />
      </div>

      <div className="mt-5 grid gap-3 sm:grid-cols-3">
        <div className="soft-panel"><span className="text-sm text-slate-500">Ціна</span><strong className="mt-1 block text-xl">{formatPrice(priceSource.price)} грн</strong><span className="text-xs text-slate-500">{priceSource.label}</span></div>
        <div className="soft-panel"><span className="text-sm text-slate-500">Потрібно пального</span><strong className="mt-1 block text-xl">{Number.isFinite(liters) ? liters.toFixed(2) : "-"} л</strong><span className="text-xs text-slate-500">{fuel?.name}</span></div>
        <div className="soft-panel"><span className="text-sm text-slate-500">Орієнтовна вартість</span><strong className="mt-1 block text-xl text-brand-700 dark:text-brand-200">{formatPrice(total)} грн</strong><span className="text-xs text-slate-500">для {distance || 0} км</span></div>
      </div>
    </Card>
  );
}
